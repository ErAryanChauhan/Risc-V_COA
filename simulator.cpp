...
private:
    ...
    int extract_reg_index(const string &reg_str) {
        if (reg_str.empty() || reg_str[0] != 'x') return -1;
    ...
    int parse_immediate(const string &str) {
        ...
        if (str.size() > 2 && str[0] == '0') {
            if (str[1] == 'x' || str[1] == 'X') return stoi(str, nullptr, 16);
            if (str[1] == 'b' || str[1] == 'B') return stoi(str.substr(2), nullptr, 2);
        }
        return stoi(str);
    }
...
public:
    ...
    void execute_instruction(Core &core, const string &instruction) {
        ...
        else if (opcode == "BNE") {  // Branch if Not Equal
        ...
        else if (opcode == "SUB") {  // Subtraction
            if (is_valid_register(rd) && is_valid_register(rs1) && is_valid_register(rs2)) {
                core.registers[rd] = core.registers[rs1] - core.registers[rs2];
            }
        } 
        else if (opcode == "SWAP") {  // Swap two registers
